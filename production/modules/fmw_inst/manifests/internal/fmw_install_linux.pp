...
)
{
  if $version == '10.3.6' {
    ...
    exec{ "Install ${title}":
      command => "${java_home_dir}/bin/java -Xmx1024m -Djava.io.tmpdir=${tmp_dir} -jar /tmp/soa_suite/fmw_12.2.1.4.0_soa.jar -waitforcompletion -silent -responseFile ${rsp_file} -invPtrLoc ${orainst_dir}/oraInst.loc",
      creates => $oracle_home_dir,
      user    => $os_user,
      group   => $os_group,
      cwd     => $tmp_dir,
