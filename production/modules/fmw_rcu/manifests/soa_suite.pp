...
class fmw_rcu::soa_suite(
  ...
  if $version == '12.2.1' {
    ...
    if $::kernel == 'windows' {
      ...
      fmw_rcu::internal::rcu_extract_windows{'soa_suite':
        version             => $version,
      ...
      $rcu_oracle_home_dir = "${tmp_dir}/rcu/rcuHome"
      fmw_rcu::internal::rcu_extract{'soa_suite':
        source_file => $source_file,
        creates_dir => $rcu_oracle_home_dir,
        os_user     => $os_user,
        os_group    => $os_group,
  ...
  $password_file = "${tmp_dir}/soa_suite_rcu_password.txt"
