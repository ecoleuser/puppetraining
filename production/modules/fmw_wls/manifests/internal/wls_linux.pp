...
)
{
  ...
  file { "${fmw_wls::orainst_dir}/oraInst.loc":
    ...
    content => template('/etc/puppetlabs/code/environments/production/modules/fmw_wls/templates/oraInst.loc'),
    mode    => '0755',
    owner   => 'root',
    group   => 'root',
  }
  ...
  file { "${fmw_wls::tmp_dir}/${wls_template}":
    ...
    content => template('/etc/puppetlabs/code/environments/production/modules/fmw_wls/templates/wls_12c.rsp'),
    mode    => '0755',
    owner   => $fmw_wls::os_user,
    group   => $fmw_wls::os_group,
  }
